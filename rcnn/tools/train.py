...
def init_params(args, sym, train_data):
    ...
    if args.resume:  # load params from previously trained model
        ...
        elif config.FRAMEWORK == '3DCE_dual_att':
            arg_params['conv_new_1_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['conv_new_1_weight'])
            arg_params['conv_new_1_bias'] = mx.nd.zeros(shape=arg_shape_dict['conv_new_1_bias'])
            arg_params['fc6_weight'] = mx.random.normal(0, 0.001, shape=arg_shape_dict['fc6_weight'])
            arg_params['fc6_bias'] = mx.nd.zeros(shape=arg_shape_dict['fc6_bias'])

            if config.ATT_TEMP_SPATIAL>0:
                arg_params['conv_new_att_s_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['conv_new_att_s_weight'])
                arg_params['conv_new_att_s_bias'] = mx.nd.zeros(shape=arg_shape_dict['conv_new_att_s_bias'])
            if config.ATT_TEMP_VERTICAL>0:
                arg_params['conv_new_att_v_weight'] = mx.random.normal(0, 0.01,shape=arg_shape_dict['conv_new_att_v_weight'])
                arg_params['conv_new_att_v_bias'] = mx.nd.zeros(shape=arg_shape_dict['conv_new_att_v_bias'])
            arg_params['cls_score_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['cls_score_weight'])
            arg_params['cls_score_bias'] = mx.nd.zeros(shape=arg_shape_dict['cls_score_bias'])
            arg_params['bbox_pred_weight'] = mx.random.normal(0, 0.001, shape=arg_shape_dict['bbox_pred_weight'])
            arg_params['bbox_pred_bias'] = mx.nd.zeros(shape=arg_shape_dict['bbox_pred_bias'])
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='3DCE Training with Mxnet')

    parser.add_argument('--exp_folder', default='exp/3DCE_7_image_3_slice_dual_att',
                        type=str, help='Path to experiment folder.yml')

    exec_args = parser.parse_args()
    config_file = cfg_from_file(os.path.join(exec_args.exp_folder, 'config.yml'))
    merge_a_into_b(config_file, config)
    config.NUM_ANCHORS = len(config.ANCHOR_SCALES) * len(config.ANCHOR_RATIOS)

    if config.FRAMEWORK[:4] != '3DCE':
        assert config.NUM_IMAGES_3DCE == 1, "Combining multiple images is only possible in 3DCE"

    default_file = cfg_from_file(os.path.join(exec_args.exp_folder, 'default.yml'))
    merge_a_into_b(default_file, default)

    model_save_folder = os.path.join(exec_args.exp_folder, 'model')
